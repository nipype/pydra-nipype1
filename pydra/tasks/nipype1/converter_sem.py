...
def converter_sem(interface_name):
    # interf = getattr(fsl, interface_name)
    input_spec = getattr(mod_sem, interf_nm+"InputSpec")()
...
if __name__ == "__main__":
    ...
    interf_nm = "BRAINSLabelStats"
    interf = getattr(mod_sem, interf_nm)
    input_spec = converter_sem(interface_name=interf_nm)

    task = pydra.ShellCommandTask(name=interf_nm, executable=interf._cmd, input_spec=input_spec)
    assert task.cmdline
    #task(plugin="cf")
